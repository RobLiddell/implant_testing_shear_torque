---
title: "rMarkdown for Data Cleaning"
author: "Robert Liddell"
date: "2022-10-27"
output: html_document
---

*Note: This could probably be written as a script next time as it doesn't really much visual output.*
...
This code is for the extraction and cleaning of data found in individual tab separated value tables saved in .txt files \## Data Extraction Out first step is extract all the data
...
extractForceData<-function(folders){
  dat<-dir(i)%>%
    ...
    mutate(testDate=ymd(str_extract(i,"2014 \\d+ \\d+")), #Extract date of testing
           ...
           femurSide=str_extract(file,"(?<=\\D)\\D"),     #Extract the side of the femur tested (L/M)
    ...
    mutate(across(animalSide:femurSide,~str_to_upper(.x)))
  return(dat)
}
...
First we extract the peak force from each data file read in. (This and previous steps could be made less memory intensive by possibly doing in the previous step). After extracting I left_join with the legend on animalSide and animalID to include the surgery date and implant type data. Afterwards the fields are modified making the different values easier to read.
...
data<-rawData%>%
  ...
  ungroup()%>%
  select(animalID,testDate,animalSide,femurSide,testMeth,force)%>% #Select relevant fields
  ...
  mutate(animalSide=factor(animalSide,levels=c("L","R"),labels=c("Left","Right")), #Re-factored names to make more readable
         femurSide=factor(femurSide,levels=c("L","M"),labels=c("Lateral","Medial")),
