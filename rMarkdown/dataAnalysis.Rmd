...
simpDat%>%
  ...
  ggplot(aes(x=factor(time),colour=implantType))+
    ...
    labs(x="Time (d)",y="Torque (Ncm)",title="Distribution of Data",caption="Figure 1. The transitions indicate 0%, 25%, 50%, 75%, and 100% quartiles")+
    impCols
...
simpDat%>%
  ...
  geom_line(linewidth=1)+
    facet_grid(rows=vars(testMethod))+
    labs(x="Time (d)",y="Growth Rate (%/Day)",title="Growth Over Time",caption="Figure 2. The rate of change in force between each time point for each implant.\nThe percent change per day indicates how much growth occured relative to\nthe peak measured anchorage.")+
    impCols+
...
bootStrap_fitCI %>% 
  ...
  impFills+
  labs(x="Time (d)",y="Torque (Ncm)",title="Asymptotic curve fit to Implant Anchorage", caption="Figure 3. Asymptotic curves fit to reverse torque data.")
  
bootStrap_results_base
